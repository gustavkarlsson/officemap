...
public class OfficeMap extends Application<OfficeMapConfiguration> {
	...
	private final HibernateBundle<OfficeMapConfiguration> hibernate = new HibernateBundle<OfficeMapConfiguration>(
			Person.class, Area.class, Reference.class, PersonReference.class, AreaReference.class, FileEntry.class) {
		@Override
	...
	private void setupJersey(final OfficeMapConfiguration config, final Environment environment) {
		final FileHandler fileHandler = new FileHandler(config.getDataDirectory(),
		...
		final FileEntryDao fileEntryDao = new FileEntryDao(hibernate.getSessionFactory());
		environment.jersey().register(new FileResource(fileEntryDao, fileHandler));
