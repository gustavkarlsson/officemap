...
public class AreaDaoTest extends AbstractDaoTest {
	...
	public void insertOtherAreaWithPersonFails() throws Exception {
		...
		final Area area = Area.Builder.fromArea(validArea1).withPersonsVararg(personRef).build();
		dao.insert(area);
		
		final Area area2 = Area.Builder.fromArea(validArea1).withPersonsVararg(personRef).withName("Other Area")
				.build();
		dao.insert(area2);
	}
	...
	public void updateOtherAreaWithPersonFails() throws Exception {
		...
		final Area area2 = Area.Builder.fromArea(validArea1).withName("Other Area").build();
		dao.insert(area2);
		final Area area2Updated = Area.Builder.fromArea(area2).withPersonsVararg(personRef).build();
		dao.update(area2Updated.getReference().getId(), area2Updated);
	}
}
